...
//  double numFreq3 = numFreq*numFreq*numFreq;
  ...
  while ( typeA[ _numCentersA ] == 'I' ) _numCentersA++;
  ...
  if ( breakDownScores )
	...
	float translate_B[ 3 ];
        ...
        if ( elecScale != 0 )
          {
            ...
            if ( !computeElecKernel( smallElectrostaticsKernel, numFreq, scale,
                                     ...
                                     pr->elecKernelDistHigh, pr->elecKernelValHigh ) ) return -1;
          }
	   ...
	   double coreCoreWeightSqrt = sqrt( pr->coreCoreWeight );
           ...
           if ( !readShapeCompGrid( sparseProfile[ 0 ], &xCenter, &yCenter, &zCenter, pr->movingMoleculeSCReRaw, pr->movingMoleculeSCImRaw ) ) return -1;
	   ...
	   if ( breakDownScores ) memcpy( sparseProfile_01[ 0 ], sparseProfile[ 0 ], numFreq * numFreq * numFreq * sizeof( FFTW_complex ) );
           ...
           if ( !readShapeCompGrid( sparseProfile[ 0 ], &xCenter, &yCenter, &zCenter, pr->staticMoleculeSCReRaw, pr->staticMoleculeSCImRaw ) ) return -1;
	   ...
	   if ( !getCenterFrequencies( numCentersA, xkA, ykA, zkA, rkA, typeA, fkA, blobbiness, alpha, numFreq, interpFuncExtent,
				       ...
				       moreFreqPlanA, NULL, true, spreadReceptorSkin ) ) return -1;
           ...
           if ( breakDownScores )
        	...
        	if ( !getCenterFrequencies( numCentersA, xkA, ykA, zkA, rkA, typeA, fkA_01, blobbiness, alpha, numFreq, interpFuncExtent,
        				    centerFrequenciesA_01, smoothSkin, smoothingFunction[ 0 ], ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], moreFreqPlanA, NULL, true, spreadReceptorSkin ) ) return -1;
        	...
        	if ( !getCenterFrequencies( numCentersA, xkA, ykA, zkA, rkA, typeA, fkA_10, blobbiness, alpha, numFreq, interpFuncExtent,
        				    centerFrequenciesA_10, smoothSkin, smoothingFunction[ 0 ], ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], moreFreqPlanA, NULL, true, spreadReceptorSkin ) ) return -1;
        	...
        	if ( !getCenterFrequencies( numCentersA, xkA, ykA, zkA, rkA, typeA, fkA_11, blobbiness, alpha, numFreq, interpFuncExtent,
        				    centerFrequenciesA_11, smoothSkin, smoothingFunction[ 0 ], ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], moreFreqPlanA, NULL, true, spreadReceptorSkin ) ) return -1;
              }

           if ( elecScale != 0 )
             {
              if ( !readElecGrid( ( FFTW_DATA_TYPE * ) elecGridB[ 0 ], &xCenter, &yCenter, &zCenter, pr->staticMoleculeElecReRaw ) ) return -1;

              if ( !getCenterElecFrequencies( numCentersA, xkA, ykA, zkA, rkA, typeA, fkAElec, blobbiness, alpha, numFreq, elecRadiusInGrids,
	                                      centerElecFrequenciesB[ 0 ], ( FFTW_DATA_TYPE * ) elecGridB[ 0 ], moreElecFreqPlanA, true, false ) ) return -1;
	   ...
	   numNonzeroGridBCells = collectNonzeroGridCells( &gridBCells, centerFrequenciesB[ 0 ], numFreq );
           ...
           if ( elecScale != 0 )
             {
              if ( !readElecGrid( ( FFTW_DATA_TYPE * ) centerElecFrequenciesB[ 0 ], &xCenter, &yCenter, &zCenter, pr->movingMoleculeElecReRaw ) ) return -1;
	    ...
	    // center A
  	    if( !center( xkAOrig, ykAOrig, zkAOrig, numCentersA, &xTrans, &yTrans, &zTrans ) ) return -1;
	    ...
	    if ( !transformAndNormalize( xkAOrig, ykAOrig, zkAOrig, radiiA, xkA, ykA, zkA, rkA, numCentersA,
				         1, 0, 0, 0, 1, 0, 0, 0, 1, 1, scale ) ) return -1;
	    ...
	    if ( !getCenterFrequencies( numCentersA, xkA, ykA, zkA, rkA, typeA, fkA, blobbiness, alpha, numFreq, interpFuncExtent,
 				   centerFrequenciesA, smoothSkin, smoothingFunction[ 0 ], ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], moreFreqPlanA, NULL, true, spreadReceptorSkin ) ) return -1;
        	...
        	if ( !getCenterFrequencies( numCentersA, xkA, ykA, zkA, rkA, typeA, fkA_01, blobbiness, alpha, numFreq, interpFuncExtent,
        				    centerFrequenciesA_01, smoothSkin, smoothingFunction[ 0 ], ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], moreFreqPlanA, NULL, false, spreadReceptorSkin ) ) return -1;

                memcpy( ourMoreFrequencies[ 0 ], sparseProfile[ 0 ], numFreq * numFreq * numFreq * sizeof( FFTW_complex ) );

        	if ( !getCenterFrequencies( numCentersA, xkA, ykA, zkA, rkA, typeA, fkA_10, blobbiness, alpha, numFreq, interpFuncExtent,
        				    centerFrequenciesA_10, smoothSkin, smoothingFunction[ 0 ], ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], moreFreqPlanA, NULL, false, spreadReceptorSkin ) ) return -1;

                memcpy( ourMoreFrequencies[ 0 ], sparseProfile[ 0 ], numFreq * numFreq * numFreq * sizeof( FFTW_complex ) );

        	if ( !getCenterFrequencies( numCentersA, xkA, ykA, zkA, rkA, typeA, fkA_11, blobbiness, alpha, numFreq, interpFuncExtent,
        				    centerFrequenciesA_11, smoothSkin, smoothingFunction[ 0 ], ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], moreFreqPlanA, NULL, false, spreadReceptorSkin ) ) return -1;
               }

            if ( elecScale != 0 )
              {
                if ( !getCenterElecFrequencies( numCentersA, xkA, ykA, zkA, rkA, typeA, fkAElec, blobbiness, alpha, numFreq, elecRadiusInGrids,
  	                                        centerElecFrequenciesB[ 0 ], ( FFTW_DATA_TYPE * ) elecGridB[ 0 ], moreElecFreqPlanA, false, false ) ) return -1;
	        ...
	        if ( !getCenterHbondFrequencies( numCentersA, xkA, ykA, zkA, rkA, hbondDistanceCutoff * scale * ( numFreq - 1 ), fkAHbond, blobbiness, alpha, numFreq,
				                 ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], moreFreqPlanA, NULL, false, false ) ) return -1;
	        ...
	        if ( !getCenterHydrophobicityFrequencies( numCentersA, xkA, ykA, zkA, rkA, fkAHydrophobicity, blobbiness, alpha, numFreq,
				                 ...
				                 ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], ourMoreFrequencies[ 0 ], moreFreqPlanA, NULL, false ) ) return -1;
                ...
                if ( twoWayHydrophobicity )
                  {
       	            if ( !getCenterHydrophobicityFrequencies( numCentersA, xkA, ykA, zkA, rkA, fkAHydrophobicityTwo, blobbiness, alpha, numFreq,
    	                                         ...
    	                                         ourMoreFrequencies[ 0 ], moreFreqPlanA, NULL, false ) ) return -1;
	    ...
	    // center B
	    if ( !center( xkBOrig, ykBOrig, zkBOrig, numCentersB, &xTrans, &yTrans, &zTrans ) ) return -1;
	    ...
	    translate_B[ 2 ] = zTrans;
            ...
            if ( randomRotate )
               {
                if ( !rotateAboutOrigin( xkBOrig, ykBOrig, zkBOrig, numCentersB, randRot ) ) return -1;
               }

            if ( !transformAndNormalize( xkBOrig, ykBOrig, zkBOrig, radiiB, xkB[ 0 ], ykB[ 0 ], zkB[ 0 ], rkB[ 0 ], numCentersB,
          	                            1, 0, 0, 0, 1, 0, 0, 0, 1, 2, scale ) ) return -1;
...
//            prT[ i ].cFilter = cFilter;
	    ...
	    if ( clusterTransRad > 0 ) prT[ i ].clustPG = new PG( 10.0, - numFreq * gridSpacing, 5.0 );
	    else prT[ i ].clustPG = NULL;
	...
	FFTW_free( fkB );
	if ( elecScale != 0 ) FFTW_free( fkBElec );
	if ( hbondWeight != 0 ) FFTW_free( fkBHbond );
        if ( ( hydrophobicityWeight != 0 ) || ( hydroPhobicPhobicWeight != 0 ) || ( hydroPhilicPhilicWeight != 0 ) || ( hydroPhobicPhilicWeight != 0 ) )
          {
            FFTW_free( fkBHydrophobicity );
            if ( twoWayHydrophobicity ) FFTW_free( fkBHydrophobicityTwo );
          }
        if ( ( simpleShapeWeight > 0 ) || ( simpleChargeWeight > 0 ) ) FFTW_free( fkBSimpleComplementarity );
