...
// Create a socket, bind to the specified port,
// and set it in listen mode to make it available for clients.
...
// Handle input on the server socket by accepting the connectioN and reading the rpc request.
...
// Accept a client connection request and create a connection to handle method calls from the client.
...
class MethodHelp : public XmlRpcServerMethod
{
	public:
		...
		std::string help()
		{
			return std::string("Retrieve the help string for a named method");
		}
};
